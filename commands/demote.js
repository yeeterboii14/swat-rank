...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('demote')
		.setDescription('Demote a user one rank.')
...
demoteResponse = await roblox.demote(Number(process.env.groupId), id)
        ...
        console.log('An error occured when running the demote command: ' + err);
    ...
    .setTitle('Success!')
      .setDescription(`I have succesfully ranked ${username} to ${demoteResponse.newRole.name}, from ${rankNameInGroup}`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
