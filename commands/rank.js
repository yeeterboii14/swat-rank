...
module.exports = {
    ...
    .addStringOption(option =>
		option.setName('rank')
			.setDescription('The name of the rank you would like the individual to be promoted to')
			.setRequired(true))
    ...
    if(Number(process.env.maximumRank) <= rankInGroup || Number(process.env.maximumRank) <= newrank){
        return interaction.reply({ embeds: [notAbleRankEmb], ephemeral: true});
              ...
              const notFoundEmbedRank = new EmbedBuilder()
    .setTitle('Oops an error has occured!')
      .setDescription(`I couldn't find ${rank} inside the group. Perhaps you misspelled?`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
    ...
    try {
        ...
        console.log('An error occured when running the setrank command: ' + err);
    ...
    .setTitle('Success!')
      .setDescription(`I have succesfully ranked ${username} to ${setRankResponse.name}, from ${rankNameInGroup}`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
