...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('deny-join')
		.setDescription('Denies a users join request.')
    .addStringOption(option =>
		option.setName('username')
			.setDescription('The roblox username of the user you want to deny into the group')
			.setRequired(true))
    ...
    try {
    acceptJoinRequestResponse = await roblox.handleJoinRequest(Number(process.env.groupId), id, false);
  } catch (err) {
    ...
    .setTitle('Success!')
      .setDescription(`I have succesfully denied ${username} from the group.`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
