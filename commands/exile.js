...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('exile')
		.setDescription('Remove a user from the group.')
    ...
    .setTitle('Oops an error has occured!')
      .setDescription(`I cannot rank exile this individual due to bot permissions.`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
    ...
    if(Number(process.env.maximumRank) <= rankInGroup || Number(process.env.maximumRank) <= rankInGroup){
    ...
    try {
        ...
        console.log('An error occured when running the exile command: ' + err);
    ...
    .setTitle('Success!')
      .setDescription(`I have succesfully exiled ${username}.`)
    .setColor(0x50C878)
          .setFooter({text: 'SWAT Automation', iconURL: process.env.image})
    .setTimestamp()
